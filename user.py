...
class User:
    ...
    def insert_sql(self):
        return '''
               insert 
                 into main.USER (EMAIL, PASSWORD_HASH, 
                                FIRST_NAME, LAST_NAME) 
               values (?, ?, ?, ?)
               '''

    def insert_tuple(self):
        pw_hash = bcrypt.hashpw(self.password.encode('utf-8'), 
                                bcrypt.gensalt())
    ...
    def update_tuple(self):
        pw_hash = bcrypt.hashpw(self.password.encode('utf-8'), 
                                bcrypt.gensalt())
        return (self.email, pw_hash, self.first_name, 
                self.last_name, self.id_user)

    def login_sql(self):
        return """
               select PASSWORD_HASH 'pw_hash', FIRST_NAME 'first', 
                      LAST_NAME 'last', ID 'id'
                 from main.USER 
                where EMAIL = ?
               """
